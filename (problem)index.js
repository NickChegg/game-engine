...
// NEW IMPORTS FOR ADDED CAPABILITIES
import { getTokenCountAsync } from "../../../tokenizers.js";
import { registerExtensionPrompt, removeExtensionPrompt, extension_prompt_roles } from "../../../extension-prompts.js";
...
window.addEventListener("message", async (event) => {
    ...
    // ---------- CAPABILITY 3: DYNAMIC SYSTEM PROMPT / AUTHOR'S NOTE ----------
        ...
        if (event.data.active === false || !event.data.text) {
            removeExtensionPrompt(promptId);
        } else {
            // Depth 0 acts perfectly like an Author's Note (near the end of the context)
    ...
    // ---------- CAPABILITY 2: DYNAMIC LOREBOOK ACTIVATION ----------
        ...
        if (event.data.active === false) {
            removeExtensionPrompt(loreId);
            return;
        }

        let loreText = event.data.text || "";
        
        // If your game only sends a keyword, it will search SillyTavern's native World Info entries
        if (!loreText && event.data.keyword) {
            const entry = world_info.find(e => 
        ...
        if (loreText) {
            // Depth 4 is ST's standard for Lorebook / World Info entries
            registerExtensionPrompt(
                loreId, 
                loreText, 
                extension_prompt_roles.SYSTEM, 
    ...
    // ---------- CAPABILITY 8: TOKEN CHECKING ----------
        ...
        try {
            const count = await getTokenCountAsync(event.data.text || "");
            if (gameWindow && !gameWindow.closed) {
                gameWindow.postMessage({
                    type: "ST_EVENT",
